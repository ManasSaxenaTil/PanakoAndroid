...
public class NFFTEventPointProcessor implements AudioProcessor {
    ...
    private void horizontalFilter() {
         ...
         * max(maxFilterSize/2,minFilterSize/2) frames and the same amount of frames in
         * the 'future' for the frame under analysis.
         */
        ...
        for (int i = frameUnderAnalysis - minFilterWindowSize / 2; i < frameUnderAnalysis
                + minFilterWindowSize / 2; i++) {
        ...
        for (int i = frameUnderAnalysis - maxFilterWindowSize / 2; i < frameUnderAnalysis
                + maxFilterWindowSize / 2; i++) {
        ...
        for (Integer frameIndex : counter.keySet()) {
            ...
            if (points.size() > maxEventPointsPerFrame) {
                ...
                for (int i = maxEventPointsPerFrame; i < points.size(); i++) {
                    ...
                    // System.out.println("Removed event point at t " + points.get(i).t + " c " +
                    // points.get(i).contrast);
                }
            }
        }
     ...
     */
    private float getFrequencyForBin(int binIndex, float[] currentPhase, float[] previousPhase2) {
        final float frequencyInHertz;
        /*
         * Use the phase delta information to get a more precise frequency estimate if
         * the phase of the previous frame is available. See Moore 1976
         * "The use of phase vocoder in computer music applications" Sethares et al.
         * 2009 - Spectral Tools for Dynamic Tonality and Audio Morphing Laroche and
         * Dolson 1999
         */
    ...
    private void packEventPointsIntoFingerprints() {
        ...
        for (int i = 0; i < eventPoints.size(); i++) {
            ...
            for (int j = 0; j < eventPoints.size(); j++) {
                ...
                if (t1 < t2 && f1 != f2 && Math.abs(t2 - t1) * frameDurationInMS > minEventPointDeltaTInMs
                        && Math.abs(t2 - t1) * frameDurationInMS < maxEventPointDeltaTInMs && f2 > minfFirstLevel
                        && f2 < maxfFirstLevel) {
