...
public enum PitchUnit {
     ...
     * By default C0 (16Hz) is used.
     */
    ABSOLUTE_CENTS("Absolute cents"),

    /**
     * Number of cents (between 0 and 1200) relative to the start of the octave. The
     * first octave starts at "absolute zero" a configured, low frequency.
     */
    RELATIVE_CENTS("Relative cents"),

    /**
     * An integer from 0 to 127 that represents the closest MIDI key. All Hz values
     * above 13289.7300 Hz are mapped to 127, all values below a certain value are
     * mapped to 0.
     */
    MIDI_KEY("MIDI key"),
     ...
     * above (13289.7300 Hz + 99,99... cents) are mapped to 127,99999... All values
     * below a certain value are mapped to 0.00.
     */
    MIDI_CENT("MIDI cent");
    ...
    /**
     * The reference frequency used to calculate absolute cent values. By default it
     * uses the same reference frequency as MIDI: C-1 = 16.35/2 Hz = 8.175.
     */
     ...
     * Hertz. E.g. if the default octave is 5 then 900 relative cents are converted
     * to 5 x 1200 + 900 = 6900 = 440Hz with the {@link PitchUnit.REF_FREQ} at
     * 8.175Hz.
     */
    ...
    public static double hertzToAbsoluteCent(final double hertzValue) {
        ...
        if (hertzValue > 0) {
            ...
            // throw new IllegalArgumentException("Pitch in Hz schould be greater than zero,
            // is " + hertzValue);
        }
    ...
    public static double midiKeyToHertz(final int midiKey) {
        if (midiKey < 0 || midiKey > 127) {
            LOG.warning("MIDI keys are defined between 0 and 127 or from " + midiKeyToHertz(0) + "Hz to "
                    + midiKeyToHertz(127) + "Hz, MIDI KEY " + midiKey + " is out of this range.");
        }
     ...
     * @exception IllegalArgumentException
     *                if the hertzValue does not fall within the range of valid MIDI
     *                key frequencies.
     */
    public static int hertzToMidiKey(final Double hertzValue) {
        ...
        if (midiKey < 0 || midiKey > 127) {
            LOG.warning("MIDI keys are defined between 0 and 127 or from " + midiKeyToHertz(0) + "Hz to "
                    + midiKeyToHertz(127) + "Hz, " + hertzValue + "does not map directly to a MIDI key.");
        }
        return midiKey;
    }
